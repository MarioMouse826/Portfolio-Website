...
export default function Projects() {
  const sectionStyle = "py-24 px-8 max-w-4xl mx-auto";
  const labelStyle = "text-sm uppercase tracking-widest text-stone-400 mb-2";
  const headingStyle = "text-3xl font-bold text-stone-900 mb-12";
  const cardStyle = [
  ...
  const descStyle = "mt-2 text-stone-500 leading-relaxed";
  const tagStyle = [
  ...
  return (
    <section id="projects" className={sectionStyle}>
      <p className={labelStyle}>Work</p>
      <h2 className={headingStyle}>Featured Projects</h2>
      <div className="space-y-8">
        {projects.map((project, i) => (
          <a key={i} href={project.link} className={cardStyle}>
